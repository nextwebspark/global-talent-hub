...
### AI Research Engine
Server-side search uses Serper API for web discovery, with a heuristic-first extraction pipeline. Company names, sectors, revenue, and employees are extracted directly from Serper search result snippets using regex/pattern matching (dot-separated lists, numbered lists, title parsing). **Gemini 2.5 Flash** (`google/gemini-2.5-flash`) via OpenRouter is the primary LLM, tried first for all AI operations (intent extraction, query optimisation, article processing, enrichment, company validation). Free OpenRouter models (Llama 3.3, Qwen3, Gemma 3, Mistral, Hermes 405B) serve as fallback if Gemini fails. Heuristic-only extraction is the final fallback — search always returns results even when all LLMs are unavailable. Executive extraction requires LLM and is skipped when unavailable. The landing page provides a simple search box without LLM model selection.
...
### AI Services
- **OpenRouter**: Free LLM models (Llama 3.3, Qwen3, Gemma 3, Mistral, Hermes 405B) for optional data enrichment and executive extraction.
