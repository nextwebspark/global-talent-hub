...
### AI Research Engine
Server-side search uses Serper API for web discovery, with a heuristic-first extraction pipeline. Company names, sectors, revenue, and employees are extracted directly from Serper search result snippets using regex/pattern matching (dot-separated lists, numbered lists, title parsing). Free OpenRouter LLM models (Llama 3.3, Qwen3, Gemma 3, Mistral, Hermes 405B) are attempted as an optional enhancement — if available they provide richer structured data, but search always returns results even when all LLMs are rate-limited. Executive extraction requires LLM and is skipped when unavailable. The landing page provides a simple search box without LLM model selection.
