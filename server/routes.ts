...
export async function registerRoutes(
  ...
  // LinkedIn profile parsing with AI
  app.post("/api/parse-linkedin", async (req, res) => {
    try {
      const { linkedinUrl, profileContent, model } = req.body;
      
      if (!profileContent || profileContent.trim().length < 50) {
        return res.status(400).json({ error: "Please paste LinkedIn profile content (at least 50 characters)" });
      }
      ...
      console.log(`[LinkedIn Parse] Extracting profile data with model: ${selectedModel}`);
      ...
      const aiResponse = await fetch('https://openrouter.ai/api/v1/chat/completions', {
        ...
        body: JSON.stringify({
          ...
          messages: [
            {
              role: 'system',
              content: `You are an expert at parsing LinkedIn profile data. Extract structured information from the provided LinkedIn profile content.

Return ONLY valid JSON (no markdown code blocks, just raw JSON) with these fields:
- name: Full name of the person
- title: Current job title
- company: Current company name (if visible)
- email: Email address (if visible, otherwise null)
- phone: Phone number (if visible, otherwise null)
- linkedin: The LinkedIn URL provided
- careerHistory: Array of objects with { company, title, startDate, endDate, description }
  - For dates, use format "YYYY-MM" or "YYYY" if only year is known
  - endDate should be null if it's current position
  - description should be a brief summary of responsibilities/achievements
- education: Array of objects with { institution, degree, fieldOfStudy, startDate, endDate }
  - Use null for missing fields
- summary: A 1-2 sentence professional summary based on their experience

Be accurate and extract only what's explicitly stated in the profile. Do not invent or assume information.`
            },
            {
              role: 'user',
              content: `Parse this LinkedIn profile and extract structured data:
...
Return the extracted data as JSON.`
      ...
      if (!aiResponse.ok) {
        ...
        console.error('[LinkedIn Parse] OpenRouter API Error:', errorText);
        return res.status(500).json({ error: "AI API request failed", message: errorText });
      }
      ...
      try {
        ...
        // Handle potential <think> blocks from reasoning models
        if (content.includes('<think>')) {
          content = content.replace(/<think>[\s\S]*?<\/think>/g, '').trim();
        }
        parsedProfile = JSON.parse(content);
      } catch (parseError) {
        console.error('[LinkedIn Parse] Failed to parse response:', aiData.choices[0].message.content);
        return res.status(500).json({ error: "Failed to parse AI response - the model returned invalid JSON" });
      }
      
      console.log(`[LinkedIn Parse] Successfully extracted profile for: ${parsedProfile.name || 'Unknown'}`);
      res.json(parsedProfile);
    } catch (error) {
      console.error("Error parsing LinkedIn profile:", error);
      res.status(500).json({ error: "Failed to parse LinkedIn profile", message: String(error) });
    }
  });
